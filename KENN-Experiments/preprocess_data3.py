...
class GenericDataset:
    def __init__(self, args):
        ...
        if args.dataset in planet_sets:
            self.dataset = torch_geometric.datasets.Planetoid(root=args.dataset, name=args.dataset,
                                                              split=args.planetoid_split)
        elif args.dataset in ogbn:
            self.dataset = PygNodePropPredDataset(name=args.dataset)

        elif args.dataset in saint_datasets:
            self.dataset = getattr(torch_geometric.datasets, args.dataset)(root=args.dataset)
        else:
    ...
    @property
    def relations(self):
        return torch.full(size=(self.data.num_edges, 1), fill_value=self.args.binary_preactivation)
...
def load_and_preprocess(args):
    ...
    if args.dataset in planet_sets:
        ...
        dataset = getattr(torch_geometric.datasets, args.dataset)(root=args.dataset, tranform=T.Compose([T.ToUndirected, T.AddAttributes]))
    else:
        raise ValueError(f'Unknown dataset specified. Use one out of: {planet_sets + ogbn + saint_datasets}')
